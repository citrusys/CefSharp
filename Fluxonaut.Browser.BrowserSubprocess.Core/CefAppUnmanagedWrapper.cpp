    ...
    namespace Browser
    {
        namespace BrowserSubprocess
        {
            ...
            void CefAppUnmanagedWrapper::OnContextCreated(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, CefRefPtr<CefV8Context> context)
            {
                ...
                auto createFluxonautBrowserObj = !_jsBindingPropertyName.empty();
                auto createFluxonautBrowserObjCamelCase = !_jsBindingPropertyNameCamelCase.empty();

                if (createFluxonautBrowserObj)
                {
                    auto fluxonautBrowserObj = CefV8Value::CreateObject(NULL, NULL);
                    fluxonautBrowserObj->SetValue(kBindObjectAsync, bindObjAsyncFunction, CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_NONE);
                    fluxonautBrowserObj->SetValue(kDeleteBoundObject, unBindObjFunction, CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_NONE);
                    fluxonautBrowserObj->SetValue(kRemoveObjectFromCache, removeObjectFromCacheFunction, CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_NONE);
                    fluxonautBrowserObj->SetValue(kIsObjectCached, isObjectCachedFunction, CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_NONE);
                    fluxonautBrowserObj->SetValue(kPostMessage, postMessageFunction, CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_NONE);
                    fluxonautBrowserObj->SetValue(kSendEvalScriptResponse, promiseHandlerFunction, CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_NONE);
                    fluxonautBrowserObj->SetValue(kRenderProcessId, CefV8Value::CreateInt(processId), CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_NONE);

                    global->SetValue(_jsBindingPropertyName, fluxonautBrowserObj, CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_READONLY);
                }

                if (createFluxonautBrowserObjCamelCase)
                {
                    auto fluxonautBrowserObjCamelCase = CefV8Value::CreateObject(NULL, NULL);
                    fluxonautBrowserObjCamelCase->SetValue(kBindObjectAsyncCamelCase, bindObjAsyncFunction, CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_NONE);
                    fluxonautBrowserObjCamelCase->SetValue(kDeleteBoundObjectCamelCase, unBindObjFunction, CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_NONE);
                    fluxonautBrowserObjCamelCase->SetValue(kRemoveObjectFromCacheCamelCase, removeObjectFromCacheFunction, CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_NONE);
                    fluxonautBrowserObjCamelCase->SetValue(kIsObjectCachedCamelCase, isObjectCachedFunction, CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_NONE);
                    fluxonautBrowserObjCamelCase->SetValue(kPostMessageCamelCase, postMessageFunction, CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_NONE);
                    fluxonautBrowserObjCamelCase->SetValue(kSendEvalScriptResponseCamelCase, promiseHandlerFunction, CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_NONE);
                    fluxonautBrowserObjCamelCase->SetValue(kRenderProcessIdCamelCase, CefV8Value::CreateInt(processId), CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_NONE);

                    global->SetValue(_jsBindingPropertyNameCamelCase, fluxonautBrowserObjCamelCase, CefV8Value::PropertyAttribute::V8_PROPERTY_ATTRIBUTE_READONLY);
                }
                ...
                //Message sent after legacy objects have been bound and the FluxonautBrowser bind async helper methods
                //have been created
                        ...
                        if (frame.get() && frame->IsValid())
                        {
                            ...
                            if (context.get() && context->Enter())
                            {
                                try
                                {
                                    ...
                                    if (success)
                                    {
                                        //If the response is a string of FluxonautBrowserDefEvalScriptRes then
                                        //we don't send the response, we'll let that happen when the promise has completed.
                                        if (result->IsString() && result->GetStringValue() == "FluxonautBrowserDefEvalScriptRes")
                                        {
                                            sendResponse = false;
                                        }
                                        else
