...
namespace Fluxonaut.Browser.Wpf.Experimental.Accessibility
{
    public class AccessibilityNode : FrameworkElementAutomationPeer
    {
        ...
        private Fluxonaut.Browser.Structs.Rect location;
        private Fluxonaut.Browser.Structs.Point scroll;
        ...
        {
            Fluxonaut.Browser.Structs.Rect location = GetLocation();
        ...
        public void Update(IDictionary<string, IValue> node)
        {
            ...
            if (node.ContainsKey("location"))
            {
                ...
                if (locationDictionary != null)
                {
                    ...
                    location = new Fluxonaut.Browser.Structs.Rect(x, y, width, height);
                }
            }
            ...
            if (node.ContainsKey("attributes"))
            {
                ...
                if (attributes != null)
                {
                    ...
                    if (attributes.ContainsKey("scrollX") && attributes.ContainsKey("scrollY"))
                    {
                        ...
                        scroll = new Fluxonaut.Browser.Structs.Point(scrollX, scrollY);
                    }
                }
            }
        }

        protected virtual Fluxonaut.Browser.Structs.Rect GetLocation()
        {
            ...
            int x = location.X;
            int y = location.Y;
            Fluxonaut.Browser.Structs.Rect offsetNodeRect = offsetNode.GetLocation();
            x += offsetNodeRect.X - offsetNode.scroll.X;
            y += offsetNodeRect.Y - offsetNode.scroll.Y;

            return new Fluxonaut.Browser.Structs.Rect(x, y, location.Width, location.Height);
        }
