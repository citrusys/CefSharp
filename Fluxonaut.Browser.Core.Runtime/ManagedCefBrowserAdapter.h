        ...
        namespace Core
        {
            ...
            public:
                    ...
                    : _isDisposed(false)
                {
                    ...
                    _javaScriptObjectRepository = gcnew Fluxonaut::Browser::Internals::JavascriptObjectRepository();
                    _javascriptCallbackFactory = gcnew Fluxonaut::Browser::Internals::JavascriptCallbackFactory(_clientAdapter->GetPendingTaskRepository());

                    if (CefSharpSettings::ConcurrentTaskExecution)
                    {
                        _methodRunnerQueue = gcnew Fluxonaut::Browser::Internals::ConcurrentMethodRunnerQueue(_javaScriptObjectRepository);
                    }
                    else
                    {
                        _methodRunnerQueue = gcnew Fluxonaut::Browser::Internals::MethodRunnerQueue(_javaScriptObjectRepository);
                    }
                ...
                {
                    Fluxonaut::Browser::Internals::IJavascriptCallbackFactory^ get();
                }

                virtual property IJavascriptObjectRepositoryInternal^ JavascriptObjectRepository
                {
                    Fluxonaut::Browser::Internals::IJavascriptObjectRepositoryInternal^ get();
                }

                virtual property IMethodRunnerQueue^ MethodRunnerQueue
                {
                    Fluxonaut::Browser::Internals::IMethodRunnerQueue^ get();
                }
            };
        }
    }
