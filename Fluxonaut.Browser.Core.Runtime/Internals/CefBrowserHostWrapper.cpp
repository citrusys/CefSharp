...
void CefBrowserHostWrapper::DragTargetDragEnter(IDragData^ dragData, MouseEvent mouseEvent, DragOperationsMask allowedOperations)
{
    ...
    auto dragDataWrapper = static_cast<Fluxonaut::Browser::Core::DragData^>(dragData);
...
Task<double>^ CefBrowserHostWrapper::GetZoomLevelAsync()
{
    ...
    if (CefCurrentlyOn(TID_UI))
    {
        ...
        Fluxonaut::Browser::Internals::TaskExtensions::TrySetResultAsync<double>(taskSource, GetZoomLevelOnUI());
...
IRequestContext^ CefBrowserHostWrapper::RequestContext::get()
{
    ...
    return gcnew Fluxonaut::Browser::Core::RequestContext(_browserHost->GetRequestContext());
}
