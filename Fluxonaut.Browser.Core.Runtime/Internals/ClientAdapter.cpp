        ...
        namespace Internals
        {
                ...
                CefRefPtr<CefClient>& client, CefBrowserSettings& settings, CefRefPtr<CefDictionaryValue>& extraInfo, bool* no_javascript_access)
            {
                ...
                auto result = handler->OnBeforePopup(
                    ...
                    (Fluxonaut::Browser::WindowOpenDisposition)target_disposition,
            ...
            bool ClientAdapter::OnAutoResize(CefRefPtr<CefBrowser> browser, const CefSize& new_size)
            {
                ...
                return handler->OnAutoResize(_browserControl, browserWrapper, Fluxonaut::Browser::Structs::Size(new_size.width, new_size.height));
            }
            ...
            bool ClientAdapter::OnCursorChange(CefRefPtr<CefBrowser> browser, CefCursorHandle cursor, cef_cursor_type_t type, const CefCursorInfo& custom_cursor_info)
            {
                ...
                auto handled = handler->OnCursorChange(_browserControl, browserWrapper, (IntPtr)cursor, (Fluxonaut::Browser::Enums::CursorType)type, customCursorInfo);
            ...
            void ClientAdapter::OnLoadStart(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, TransitionType transitionType)
            {
                ...
                if (!browser->IsPopup() || _browserControl->HasParent)
                {
                    _browserControl->OnFrameLoadStart(gcnew FrameLoadStartEventArgs(browserWrapper, %frameWrapper, (Fluxonaut::Browser::TransitionType)transitionType));
                }

                auto handler = _browserControl->LoadHandler;
                if (handler != nullptr)
                {
                    handler->OnFrameLoadStart(_browserControl, gcnew FrameLoadStartEventArgs(browserWrapper, %frameWrapper, (Fluxonaut::Browser::TransitionType)transitionType));
                }
            }
                ...
                CefRequestHandler::WindowOpenDisposition targetDisposition, bool userGesture)
            {
                ...
                return handler->OnOpenUrlFromTab(_browserControl, browserWrapper, %frameWrapper, StringUtils::ToClr(targetUrl), (Fluxonaut::Browser::WindowOpenDisposition)targetDisposition, userGesture);
            }
            ...
            bool ClientAdapter::OnDragEnter(CefRefPtr<CefBrowser> browser, CefRefPtr<CefDragData> dragData, DragOperationsMask mask)
            {
                ...
                return handler->OnDragEnter(_browserControl, browserWrapper, %dragDataWrapper, (Fluxonaut::Browser::Enums::DragOperationsMask)mask);
            }
            ...
            bool ClientAdapter::GetAudioParameters(CefRefPtr<CefBrowser> browser, CefAudioParameters & params)
            {
                ...
                auto browserWrapper = GetBrowserWrapper(browser->GetIdentifier(), browser->IsPopup());
                auto parameters = new AudioParameters((Fluxonaut::Browser::Enums::ChannelLayout)params.channel_layout, params.sample_rate, params.frames_per_buffer);
            ...
            void ClientAdapter::OnAudioStreamStarted(CefRefPtr<CefBrowser> browser, const CefAudioParameters& params, int channels)
            {
                ...
                {
                    auto browserWrapper = GetBrowserWrapper(browser->GetIdentifier(), browser->IsPopup());
                    AudioParameters parameters((Fluxonaut::Browser::Enums::ChannelLayout)params.channel_layout, params.sample_rate, params.frames_per_buffer);
            ...
            bool ClientAdapter::OnProcessMessageReceived(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, CefProcessId source_process, CefRefPtr<CefProcessMessage> message)
            {
                ...
                else if (name == kEvaluateJavascriptResponse || name == kJavascriptCallbackResponse)
                {
                    ...
                    if (pendingTask != nullptr)
                    {
                        ...
                        Fluxonaut::Browser::Internals::TaskExtensions::TrySetResultAsync<JavascriptResponse^>(pendingTask, response);
                    }
