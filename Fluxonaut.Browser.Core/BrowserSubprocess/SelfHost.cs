...
namespace Fluxonaut.Browser.BrowserSubprocess
{
    /// <summary>
    ...
    ///
    ///     var exitCode = Fluxonaut.Browser.BrowserSubprocess.SelfHost.Main(args);
    ///
    ///     if (exitCode >= 0)
    ///     {
    ///       return exitCode;
    ...
    public class SelfHost
    {
        ...
        public static int Main(string[] args)
        {
            ...
            {
                browserSubprocessDllPath = Path.Combine(Path.GetDirectoryName(typeof(Fluxonaut.Browser.Core.BrowserSettings).Assembly.Location), "Fluxonaut.Browser.BrowserSubprocess.Core.dll");
            }
            var browserSubprocessDll = System.Runtime.Loader.AssemblyLoadContext.Default.LoadFromAssemblyPath(browserSubprocessDllPath);
#else
            var browserSubprocessDllPath = Path.Combine(Path.GetDirectoryName(typeof(Fluxonaut.Browser.Core.BrowserSettings).Assembly.Location), "Fluxonaut.Browser.BrowserSubprocess.Core.dll");
            var browserSubprocessDll = System.Reflection.Assembly.LoadFrom(browserSubprocessDllPath);
#endif
            var browserSubprocessExecutableType = browserSubprocessDll.GetType("Fluxonaut.Browser.BrowserSubprocess.BrowserSubprocessExecutable");
